...
const PosterList: React.FC<Props> = ({ title, endpoint, type }) => {
  ...
  const posterList = isLoadingList ? (
    ...
    list.map((item) => (
      <Link href={`${type}/${item.id}`} key={item.id}>
        <div className="w-60 h-80 relative flex flex-col bg-gray-200 items-center justify-center mb-3 text-gray-400 cursor-pointer rounded-xl overflow-hidden shadow-lg">
          {item.poster_path ? (
            <LazyImage
          ...
          <div className="absolute w-60 truncate text-white bottom-0 px-4 py-2 bg-gradient-to-t from-red-500">
            {item.title}
          </div>
        </div>
      </Link>
  ...
  return (
    <>
      ...
      <div className="grid grid-flow-col gap-4 overflow-x-auto mt-3 pb-2 scrollbar-thin scrollbar-thumb-rounded-lg scrollbar-thumb-gray-300 scrollbar-track-gray-200 dark:scrollbar-track-gray-700 dark:scrollbar-thumb-gray-400 hover:scrollbar-thumb-gray-400 dark:hover:scrollbar-thumb-gray-300">
        {posterList}
      </div>
    </>
  );
